...
@interface DDAlertAction ()
@property (nonatomic, strong) NSString *title;
@property (nonatomic, assign) DDAlertActionStyle style;
@property (nonatomic, copy  ) void (^handler)(DDAlertAction *);
@end

@implementation DDAlertAction

+ (instancetype)actionWithTitle:(NSString *)title style:(DDAlertActionStyle)style handler:(void (^)(DDAlertAction *))handler
{
    return [[self alloc]initWithTitle:title style:style handler:handler];
}

- (instancetype)initWithTitle:(NSString *)title style:(DDAlertActionStyle)style handler:(void (^)(DDAlertAction *))handler
{
    if (self = [super init]) {
        _title = title;
        _style = style;
...
{
    DDAlertAction *action = [[self class]allocWithZone:zone];
...
- (UIColor *)buttonBgColorWithStyle:(DDAlertActionStyle)style
{
    switch (style) {
        case DDAlertActionStyleDefault:
            return _buttonDefaultBgColor;
        case DDAlertActionStyleCancel:
            return _buttonCancelBgColor;
        case DDAlertActionStyleDestructive:
...
- (void)addOtherViews
{
    ...
    topImgView.image = [UIImage imageNamed:@"regret"];
    [_bottomContentView addSubview:topImgView];
    _topImgView = topImgView;
    
    UIButton *closeBtn = [[UIButton alloc]init];
    [closeBtn setImage:[UIImage imageNamed:@"alertClose"] forState:UIControlStateNormal];
    [closeBtn addTarget:self action:@selector(close:) forControlEvents:UIControlEventTouchUpInside];
    [_bottomContentView addSubview:closeBtn];
    _closeBtn = closeBtn;
}
...
- (void)layoutContentViews
{
    ...
    [self addConstraintWithView:_bottomContentView
                        topView:self
                       leftView:self
                     bottomView:self
                      rightView:self
...
//    UIEdgeInsetsMake(<#CGFloat top#>, <#CGFloat left#>, <#CGFloat bottom#>, <#CGFloat right#>)
    ...
    [self addConstraintWithView:_textContentView
                        topView:self
                       leftView:self
                     bottomView:nil
                      rightView:self
                      edgeInset:UIEdgeInsetsMake(_contentViewSpace, _textLabelContentViewEdge, 0, -_textLabelContentViewEdge)];
    
    // textFieldContentView
    _textFieldContentView.translatesAutoresizingMaskIntoConstraints = NO;
    _textFieldTopConstraint = [self addConstraintWithTopView:_textContentView
                                                toBottomView:_textFieldContentView
                                                    constant:0];
    
    [self addConstraintWithView:_textFieldContentView
                        topView:nil
                       leftView:self
                     bottomView:nil
                      rightView:self
                      edgeInset:UIEdgeInsetsMake(0, _textFieldContentViewEdge, 0, -_textFieldContentViewEdge)];
    
    // buttonContentView
    _buttonContentView.translatesAutoresizingMaskIntoConstraints = NO;
    
    _buttonTopConstraint = [self addConstraintWithTopView:_textFieldContentView
                                             toBottomView:_buttonContentView
                                                 constant:_buttonContentViewTop];
    
    [self addConstraintWithView:_buttonContentView
                        topView:nil
                       leftView:self
                     bottomView:self
                      rightView:self
                      edgeInset:UIEdgeInsetsMake(0, _buttonContentViewEdge, -_contentViewSpace, -_buttonContentViewEdge)];
}

- (void)layoutOtherViews
{
    if (!_topImgView.translatesAutoresizingMaskIntoConstraints) {
        // layout done
        return;
    }
    
    _topImgView.translatesAutoresizingMaskIntoConstraints = NO;
    [_topImgView addConstraintWidth:50 height:50];
    [_bottomContentView addConstraintWithView:_topImgView
                                      topView:_bottomContentView
                                     leftView:_bottomContentView
                                   bottomView:_textContentView
                                    rightView:_bottomContentView
                                    edgeInset:UIEdgeInsetsMake(0, _buttonContentViewEdge, _contentViewSpace, _buttonContentViewEdge)];
    
    _closeBtn.translatesAutoresizingMaskIntoConstraints = NO;
    [_closeBtn addConstraintWidth:30 height:30];
    [_bottomContentView addConstraintWithView:_closeBtn
                                      topView:_bottomContentView
                                     leftView:nil
                                   bottomView:nil
                                    rightView:_bottomContentView
...
- (void)layoutTextLabels
{
    ...
    [_textContentView addConstraintWithView:_titleLable
                                    topView:_textContentView
                                   leftView:_textContentView
                                 bottomView:nil
                                  rightView:_textContentView
                                  edgeInset:UIEdgeInsetsZero];
    
    // message
    _messageLabel.translatesAutoresizingMaskIntoConstraints = NO;
    [_textContentView addConstraintWithTopView:_titleLable
                                  toBottomView:_messageLabel
                                      constant:_textLabelSpace];
    [_textContentView addConstraintWithView:_messageLabel
                                    topView:nil
                                   leftView:_textContentView
                                 bottomView:_textContentView
                                  rightView:_textContentView
...
{
    DDAlertAction *action = _actions[button.tag - kButtonTagOffset];
