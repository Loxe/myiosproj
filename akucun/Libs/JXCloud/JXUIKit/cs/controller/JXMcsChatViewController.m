...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = JXColorWithRGB(238, 238, 238);//修改1
    [self setupRightItem];
    WEAKSELF;
...
//    self.tableView.backgroundColor = JXColorWithRGB(240, 240, 240);
    self.tableView.backgroundColor = JXColorWithRGB(238, 238, 238);//修改2
...
- (void)dealloc {
    @try {
        [[JXMCSUserManager sharedInstance] removeObserver:self
                                               forKeyPath:@"unreadMessageCount"];
...
//        label.attributedText = [self titleStringWith:title andImage:nil]; //修改6
    } else {
        label.attributedText = [self titleStringWith:title andImage:nil];
//        NSString *offlineStr = [NSString stringWithFormat:@"%@(结束)",title];
//        label.attributedText = [self titleStringWith:offlineStr andImage:nil];//修改7
    }
    [label sizeToFit];
}
...
- (void)setupToolbarItemsWithStatus:(JXMCSUserStatus)status {
    ...
    self.messageToolbar.isHiddenRecordBtn = NO;
...
// 在线留言
- (void)leaveMessageOnline:(voidBlock)finished {
    [[JXMCSUserManager sharedInstance] leaveMessageOnlineForUI:self workgroup:self.workgroup finished:finished];
    [sClient.mcsManager cancelWait];
}

- (void)leaveMessageOnline {
    [[JXMCSUserManager sharedInstance] leaveMessageOnlineForUI:self workgroup:self.workgroup finished:nil];
    [sClient.mcsManager cancelWait];
}
...
//        [self setupToolbarItemsWithStatus:JXMCSUserStatusInService];
...
//        self.messageToolbar.hidden = hideToolbar;
        self.messageToolbar.hidden = NO;//改,为了解决问题，临时设置
    });
}
...
//                    [self.navigationController popToRootViewControllerAnimated:YES];
            ...
            case JXErrorTypeMcsLeaveQueue: {
                ...
                if (self.hasRobot) {
                    ...
                    self.messageToolbar.hidden = NO;//改,为了解决问题，临时设置为NO,原来为YES
                }
    ...
    if (alertView.tag == transAgentTypeTag) {
        ...
        if (buttonIndex == 0) {
            ...
            [self leaveMessageOnline:^{
                if (!self.hasRobot) {
                    //[self.navigationController popToRootViewControllerAnimated:NO];//修改，不能全部pop掉
...
- (void)popSelf {
    if (self.navigationController) {
        [[NSNotificationCenter defaultCenter] postNotificationName:NOTIFICATION_FORWARD_SHOW object:nil]; //修改，发通知让“转发按钮显示”
