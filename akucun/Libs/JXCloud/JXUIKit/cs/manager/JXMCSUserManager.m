...
- (void)loginWithCallback:(JXUserActiveResponseBlock)loginResponse {
    JX_APPKEY = [JXIMClient appkey];
    JXDebugAssert(JX_APPKEY);

    NSDictionary *clientConfig = [JXIMClient clientConfig];
    NSString *cid = [clientConfig objectForKey:@"cid"];
    if ([cid length]) {
        if (![cid isEqualToString:[self mcsCid:JX_APPKEY]]) {
            [self clearAccount];
        }
        ...
        NSString *mcsAccount = [self mcsAccountForApp:JX_APPKEY];
...
// UI加载在线留言
- (void)leaveMessageOnlineForUI:(UIViewController *)vc workgroup:(JXWorkgroup *)workgroup finished:(voidBlock)finished {
 ...
 */
- (void)clearAccount {
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:JX_APPKEY];
...
- (void)saveAccount:(NSString *)account {
    [[NSUserDefaults standardUserDefaults] setObject:account forKey:JX_APPKEY];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
- (void)saveCid:(NSString *)cid {
    ...
    NSString *cidKey = [NSString stringWithFormat:@"%@|cid", JX_APPKEY];
...
- (void)didLoginFailureWithError:(JXError *)error {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (error.errorCode == JXErrorTypeUserRemoved ||
            ...
            [self clearAccount];
            [self loginWithAppKey:JX_APPKEY responseObject:self.loginResponse];
            return;
        }
        self.loginResponse(NO, error);
    });
