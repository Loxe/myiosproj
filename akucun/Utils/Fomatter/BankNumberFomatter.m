...
@implementation UITextField (DDRange)
...
/** --------------NumberFomatter----------------------------------- */
@implementation BankNumberFomatter
...
- (void)numberField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    //校验卡号只能是数字，且不能超过19位
    if ( (string.length != 0 && ![self isValidNumbers:cardNo]) || cardNo.length > 19) {
        return;
    }
    //获取【光标右侧的数字个数】
