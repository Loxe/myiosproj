...
-(void)show{
    ...
    self.frame = CGRectMake(self.fixedX, -self.standardHeight, self.fixedWidth, self.standardHeight+10);
    
    WEAK_SELF(weakSelf);
    [UIView animateWithDuration:_maker.animationDuration animations:^{
        weakSelf.frame = CGRectMake(weakSelf.fixedX, weakSelf.fixedY, weakSelf.fixedWidth, weakSelf.standardHeight+10);//weakSelf.standardHeight
...
//    self.frame = CGRectMake(self.fixedX, self.fixedY, self.fixedWidth, self.standardHeight);
    self.frame = CGRectMake(self.fixedX, self.fixedY, self.fixedWidth, self.standardHeight+10);
}
...
-(CGFloat)fixedX{
    return ([self isiPhoneX] && ![self isPortrait]) ? 128 : 10;//0
}

-(CGFloat)fixedY{
    return ([self isiPhoneX] && [self isPortrait]) ? 33 : 10;//0
}

-(CGFloat)fixedWidth{
    return ([self isiPhoneX] && ![self isPortrait]) ? 556 : ScreenWidth-20;//ScreenWidth
}
