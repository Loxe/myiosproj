...
@interface AVCaptureBackController ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate>
...
@implementation AVCaptureBackController
...
//
//            NSLog(@"是否包含国徽：%d, facePathRect: %@, faceRegion: %@",CGRectContainsRect(self.faceDetectionFrame, faceRegion),NSStringFromCGRect(self.faceDetectionFrame),NSStringFromCGRect(faceRegion));
//
...
//                NSLog(@"延迟拍照");
//                // 身份证信息识别
//                [weakSelf IDCardRecognit:imageBuffer];
//            });
            
            // 方法三:
            //[self performSelector:@selector(IDCardRecognit:) withObject:nil afterDelay:3.0];
    ...
    if (CVPixelBufferLockBaseAddress(imageBuffer, 0) == kCVReturnSuccess) {
        ...
        if (ret <= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                // 拍完将框变成绿色
                self.IDCardScaningView.IDCardScanningWindowLayer.borderColor = [UIColor greenColor].CGColor;
            });
            ...
            if (iDInfo) {// 读取到身份证信息，实例化出IDInfo对象后，截取身份证的有效区域，获取到图像
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [NSThread sleepForTimeInterval:1.0f];//延迟1s
