...
/** 扫描view */
#define scanViewW self.frame.size.width
#define scanViewH self.frame.size.height

/** 二维码 */
// 扫描内容的 W 值
#define scanBorderW 0.7*scanViewW
// 扫描内容的 H 值
#define scanBorderH scanBorderW
// 扫描内容的 x 值
#define scanBorderX 0.15*scanViewW
// 扫描内容的 Y 值
#define scanBorderY 0.5*(scanViewH - scanBorderW)

/** 条码 */
// 扫描内容的 W 值
#define scanBarcodeW 0.8*scanViewW
// 扫描内容的 H 值
#define scanBarcodeH 0.4*scanBarcodeW
// 扫描内容的 x 值
#define scanBarcodeX 0.1*scanViewW
// 扫描内容的 Y 值
#define scanBarcodeY 0.5*(scanViewH - scanBarcodeW)+0.3*scanBarcodeW
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_scanningType == ScanningTypeBarCode) {
        borderW = scanBarcodeW;
        borderH = scanBarcodeH;
        borderX = scanBarcodeX;
        borderY = scanBarcodeY;
    }
...
- (void)addTimer {
    ...
    if (self.scanningAnimationStyle == ScanningAnimationStyleGrid) {
        ...
        if (_scanningType == ScanningTypeBarCode) {
            scanninglineW = scanBarcodeW;
            scanninglineX = scanBarcodeX;
            scanninglineY = scanBarcodeY;
        }
...
- (void)beginRefreshUI {
    ...
    if (self.scanningAnimationStyle == ScanningAnimationStyleGrid) {
        ...
        CGFloat scanW = _scanningType == ScanningTypeBarCode ? scanBarcodeW : scanBorderW;
        if (flag) {
            //frame.origin.y = scanBorderY;
            frame.origin.y = _scanningType == ScanningTypeBarCode ? scanBarcodeY : scanBorderY;
            flag = NO;
            [UIView animateWithDuration:self.animationTimeInterval animations:^{
                frame.origin.y += 2;
                _scanningline.frame = frame;
...
//                CGFloat scanContent_MaxY = scanBorderY + scanViewW - 2 * scanBorderX;
