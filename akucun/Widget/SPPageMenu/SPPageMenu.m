...
- (void)showBadgeAt:(NSInteger)index
{
    ...
    SPItem *button = [self.buttons objectAtIndex:index];
...
- (void)initialize {
    ...
    dividingLine.backgroundColor = COLOR_SEPERATOR_LINE;
...
- (void)buttonInPageMenuClicked:(SPItem *)sender {
    ...
    CGFloat fromIndex = self.selectedButton ? self.selectedButton.tag-tagBaseValue : (-1);
...
- (void)layoutSubviews {
    ...
    CGFloat dividingLineH = (self.dividingLine.hidden || self.dividingLine.alpha < 0.01) ? 0 : kPIXEL_WIDTH;
    ...
    [self.buttons enumerateObjectsUsingBlock:^(SPItem *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        button.badgeCenterOffset = CGPointMake(0, 12);
    }];
