...
- (void)callDelegateMethodWithPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr {
    ...
    if (tzImagePickerVc.allowPickingVideo) {
        if ([tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didFinishPickingVideo:sourceAssets:)]) {
            [tzImagePickerVc.pickerDelegate imagePickerController:tzImagePickerVc didFinishPickingVideo:photos[0] sourceAssets:assets[0]];
        }
    } else {
...
//        #warning modify area
        ...
        //return cell;
        if (tzImagePickerVc.allowPickingVideo && tzImagePickerVc.allowPickingImage) {
            tzImagePickerVc.takePictureImageName = @"takePicture.png";
        } else if (!tzImagePickerVc.allowPickingVideo && tzImagePickerVc.allowPickingImage) {
            tzImagePickerVc.takePictureImageName = @"takePicture.png";
        } else if (tzImagePickerVc.allowPickingVideo && !tzImagePickerVc.allowPickingImage) {
            tzImagePickerVc.takePictureImageName = @"takeVideo.png";//takePicture.png
        } else if (!tzImagePickerVc.allowPickingVideo && !tzImagePickerVc.allowPickingImage) {
            tzImagePickerVc.takePictureImageName = @"takePicture.png";//takeVideo.png
        }
        TZAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TZAssetCameraCell" forIndexPath:indexPath];
        cell.imageView.image = [UIImage imageNamedFromMyBundle:tzImagePickerVc.takePictureImageName];
        return cell;
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied) && iOS7Later) {
        ...
        if (tzImagePickerVc.allowPickingVideo) { // 调用视频
            YXVideoCaptureViewController *videoVC = [[YXVideoCaptureViewController alloc] init];
