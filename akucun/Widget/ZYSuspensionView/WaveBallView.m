...
- (void)initialize
{
    ...
    _scaleCount = 0;  // 控制外面刻度的个数100
    
    a = 1.5;
    b = 0;
    increase = NO;
    
    _frontWaterColor = CLEAR_COLOR; //RGBCOLOR(69, 151, 199);
    _backWaterColor  = COLOR_MAIN;
    _waterBgColor    = RGBCOLOR(102, 102, 102);
    ...
    //    _waterBgColor = [UIColor redColor];
    _percent = 0.0;
    _title   = @"";
...
- (void)drawWave:(CGContextRef)context {
    ...
    for(float x = 0; x <= waveRect.size.width; x++){
        ...
        //前浪绘制
        frontY = a * sin( x / 80 * M_PI + 4 * b / M_PI ) * amplitude + currentLinePointY;
        ...
        //后波浪绘制
        // 里面的x/80,80为外面的控件的宽高，用来控制波峰与波谷的间距的
        backY = a * cos( x / 80 * M_PI + 3 * b / M_PI ) * amplitude + currentLinePointY;
...
//    NSMutableAttributedString *attriButedText = [self formatBatteryLevel:_title];
//    CGRect textSize = [attriButedText boundingRectWithSize:CGSizeMake(60, 10000) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading context:nil];

    NSMutableAttributedString *attriLabelText = [self titleFormatLabel:_title];
    CGRect labelSize = [attriLabelText boundingRectWithSize:CGSizeMake(60, 10000) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading context:nil];
    
    CGPoint textPoint = CGPointMake(fullRect.size.width / 2 - labelSize.size.width / 2, labelSize.size.height / 2);
//    CGPoint labelPoint = CGPointMake(fullRect.size.width / 2 - labelSize.size.width / 2, textPoint.y + textSize.size.height);
    
//    [attriButedText drawAtPoint:textPoint];
    [attriLabelText drawAtPoint:textPoint];
 ...
 *  @return 相关参数
 */
-(NSMutableAttributedString *) titleFormatLabel:(NSString*)text
{
    UIColor *textColor = [UIColor whiteColor];
    NSMutableAttributedString *attrText;
    NSMutableParagraphStyle *paragrahStyle = [[NSMutableParagraphStyle alloc] init];
    ...
    UIFont *font = [UIFont boldSystemFontOfSize:8]; //[UIFont systemFontOfSize:14];
...
- (void)setBackWaterColor:(UIColor *)backWaterColor {
    ...
    _frontWaterColor = [backWaterColor colorWithAlphaComponent:.6f];
//    [self initDrawingRects];
}
