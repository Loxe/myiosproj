...
@interface YXPhotoCaptureViewController ()<AVCaptureFileOutputRecordingDelegate>
{
...
@implementation YXPhotoCaptureViewController
...
//    _rightBtnFrame = CGRectOffset(_centerBtnFrame, 32 + btnW, 0);
    [self restoreBtn];
    _cameraBtn.frame = CGRectMake((APP_WIDTH - 67) * 0.5, CGRectGetMaxY(_viewContainer.frame) + 50, 67, 67);
...
//    _importBtn.backgroundColor = [UIColor redColor];
//    _retakeBtn.backgroundColor = [UIColor greenColor];
}
- (void)prepareUI {
    _closeBtn = [[UIButton alloc] init];
    [_closeBtn setImage:[UIImage imageNamed:@"back"] forState:UIControlStateNormal];
...
//    [imagePickerVc showProgressHUD];
        ...
        [[TZImageManager manager] savePhotoWithImage:_imageView.image completion:^(NSError *error){
            if (!error) {
                self.reloadBlock();
